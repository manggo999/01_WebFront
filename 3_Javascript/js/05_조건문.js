...
function check1() {
    ...
    if(value > 0) { // value가 0도다 커서 true인 경우
        alert("양수입니다.");
    }
...
function check2() {
    ...
    // 0~100 사이 난수
    const radomNumber = Math.floor(Math.random() * 101);

    if(radomNumber % 2 == 1) {  // 홀수
        alert(`${radomNumber}는 홀수 입니다.`);
    } 
        else{ // 홀수가 아닌 경우 == 짝수
        alert(`${radomNumber}는 짝수 입니다.`);
    }
}
...
function check3() {
    ...
    //난수 발생
    const radomNumber = Math.floor(Math.random() * 7) -3;

    let message = radomNumber + "은/는" ;

    if(radomNumber == 0) {
        message += "0 입니다."; // 복합 대입 연산자 활용

    } else if(radomNumber > 0) { // 양수인 경우
        message +=  "양수 입니다";
...
function check4() {
    ...
    } else if(age <= 19) { // 청소년 14 ~ 19세 까지 (0~13세 까지는 위에서 걸러짐)
        alert("청소년");
