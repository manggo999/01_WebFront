...
number3 = 500; // 기존 변수 number3에 새로운 값 덮어쓰기(재대입_)
console.log("number3 : " + number3);

//--------------------------------------------------------------

//3. consst (상수, constant, 항[상] 같은 [수])
...
// -> 상수에 새로운 값을 대입할 수 없는데, 대입하려고해서 에러남
...
// block 레벨 scope (let, const)
// : let과 const로 선언된 변수/상수는 변수가 선언된 블록(중괄호{})
// 내부에서만 유효하다 라는 것을 의미
//즉, 코드 레벨 내부에서 선언한 변수(let, const)는 해당 블록 외부에서는 
//접근 할 수 없다.
...
//console.log(num1); // ReferenceError: num1 is not defined -> 안되는 이유
//console.log(num2); // ReferenceError: num1 is not defined -> 안되는 이유
...

{
    ...
    example(); // 함수의 호출. 호출을 해야 함수가 수행되면서 함수내에서 작성된 코드가 실행됨
    ...
    // 1. var 의 호이스팅
    // : var로 선언된 변수는 
    // 선언만 해당 스코프의 최상단으로 호이스팅(상단으로 끌어올려지는 것)됨
    ...
    // 이러한 특성들 때문에 let/const는 보다 예측 가능한 코드를 작성할 수 있다.
...
//undefined - numbers[3] 에는 아무값도 할당되지 않아서 undefined가 된다.
...
// V(vaiue) : k에 대응되는 값 (변수에 대입되는 값 비슷)
